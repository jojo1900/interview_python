...
python函数的参数传递是赋值传递，或者叫做对象的引用传递。一个对象，可以被多次引用，例如下面的例子。显示创建一个1的对象，然后a是对这个对象的引用，传参时，传递的是一个新的指向1这个对象的引用。

需要注意的是，这里的赋值或对象的引用传递，不是指向一个具体的内存地址，而是指向一个具体的对象。

如果对象是可变的，在函数中对象变化后，会影响所有指向该对象的引用。
如果是不可变的， a+1则会创建一个新的对象，该引用指向新的对象。
...
>>>b[0].append(3)
>>>print(b,a)
[[1,2,3],(10,11),100],[[1,2,3],(10,11)]

```
即，重新分一块内存给b,b中的元素是A中元素的引用。
...
- is是对比内存地址,看是否是同一个对象，==是对比值，判断值是否相等。
